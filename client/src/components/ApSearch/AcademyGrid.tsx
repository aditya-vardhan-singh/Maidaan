...
export function AcademyGrid() {
    ...
    const fetchAcademies = async () => {
      try {
        ...
        if (
          ...
          response.data.tournaments.length === 0
        ) {
  ...
  const renderCard = (academy: (typeof academies)[0]) => (
    <Card key={academy.name} withBorder padding="lg" className={classes.card}>
      ...
      <Text c="dimmed">{academy.registeredStudents} Students</Text>

      <Text mt="sm" mb="md" c="dimmed">
        Timings: {academy.timings} <br /> • Sports: <br />{" "}
        {academy.sports.join(", ")} <br />• Location: <br /> {academy.location}
      </Text>
